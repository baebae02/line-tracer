...
# 저장할 디렉토리를 지정합니다.
output_directory = "[Forward]training_images_without_noise(light)"
os.makedirs(output_directory, exist_ok=True)

def draw_toward_without_noise(index):
    # 이미지의 크기를 설정합니다.
    image_width = 16
    image_height = 16
    ...
    curve_amplitude = 3 # 커브 진폭
    curve_direction = 0  # 오른쪽으로 커브
    ...
    for y in range(image_height):
        ...
        if 0 <= x < image_width:
            cv2.line(image, (x, y), (x, y), (128, 128, 128), line_thickness)  # 검정색 선 (0,0,0) 회색 선 (128, 128, 128)
    ...
    # 이미지를 저장합니다.
    image_filename = os.path.join(output_directory, f"line_image_toward_without_noise_{index}.png")
...
for i in range(1000):
    data_set = draw_toward_without_noise(i)
